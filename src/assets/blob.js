...
function generateFibonacciSphere1(N, jitter) {
  ...
  for (let k = 0, long = 0, z = 1 - dz / 2; k !== N; k++, z -= dz) {
    ...
    latDeg +=
      jitter * _randomLat[k] * (latDeg - (Math.asin(Math.max(-1, z - (dz * 2 * Math.PI * r) / s)) * 180) / Math.PI)
...
function addSouthPoleToMesh(southPoleId, { triangles, halfedges }) {
  ...
  for (let i = 0, s = firstUnpairedSide; i < numUnpairedSides; i++, s = pointIdToSideId[newTriangles[s_next_s(s)]]) {
...
function pushCircumcenterOfTriangle(out, ax, ay, az, bx, by, bz, cx, cy, cz) {
  ...
  let a = [ax, ay, az],
    ...
    toCircumsphereCenter = vec3.scale([], numerator, 1 / (2 * vec3.squaredLength(abXac))),
